Total Points    : 7 + 12 = 19
Points Achieved : 7 + 12 = 19
...
#include <bits/stdc++.h>
using namespace std;
bool ret(pair<pair<int,int>,int>val1,pair<pair<int,int>,int>val2) 
{
...
int main() 
{
    ...
    for(int test=1;test<=t;test++) 
    {
        cout<<"Case #"<<test<<": ";
        int n;
        vector<int>v1((24*60)+1,0);
        vector<int>v2((24*60)+1,0);
        vector<int>v3((24*60)+1,0);
        vector<int>v4((24*60)+1,0);
        cin>>n;
        bool possible= true;
        ...
        for(int i=0;i<n;i++) 
        {
            cin>>a;
            cin>>b;
            vec[i]=make_pair(make_pair(a,b),i);
        }
        sort(vec.begin(),vec.end(), ret);
        bool impossible=false;
        int cameron=0,jamie=0;
        for(int i=0;i<n;i++) 
        {
            int first=vec[i].first.first;
            int second=vec[i].first.second;
            int index=vec[i].second;
            if(jamie==0||jamie<=first) 
            {
                jamie=second;
                out[index]='J';
                impossible=false;
            ...
            else if(cameron==0||cameron<=first) 
            {
                cameron=second;
                out[index]='C';
                impossible=false;
